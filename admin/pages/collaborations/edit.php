 ...
 * Loads existing collaboration and pre-fills the form
 */

// Get the collaboration ID from URL
$collabId = (int) ($_GET['id'] ?? 0);
if ($collabId <= 0) {
    echo '<div class="alert alert-error">Ungültige Kollaborations-ID.</div>';
    return;
}
?>
<div class="page-header">
    <div class="page-header-content">
        <nav class="breadcrumb"><a href="?page=collaborations/index">Kollaborationen</a> <span>›</span>
            <span>Bearbeiten</span>
        </nav>
        <h1>Kollaboration bearbeiten</h1>
        <p class="page-subtitle">Aktualisieren Sie die Daten dieser Kollaboration</p>
    </div>
    <div class="page-header-actions">
        <a href="?page=collaborations/index" class="btn">Abbrechen</a>
        <button class="btn" onclick="CollabEdit.saveAsDraft()"><span class="material-symbols-rounded">draft</span> Als
            Entwurf</button>
        <button class="btn btn-primary" onclick="CollabEdit.save()"><span class="material-symbols-rounded">save</span>
            Speichern</button>
    </div>
</div>
...
<div id="loadingState" style="text-align:center;padding:60px;">
    <span class="material-symbols-rounded spinning" style="font-size:36px;color:var(--text-muted);">sync</span>
    <p style="color:var(--text-muted);margin-top:12px;">Lade Kollaboration...</p>
</div>

<!-- Step Navigation (hidden until loaded) -->
...
<form id="collabForm" class="product-form" style="display:none;">
    <input type="hidden" name="id" value="<?= $collabId ?>">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-textarea" name="short_description" id="shortDesc" rows="2"
                            placeholder="Kurze Beschreibung..."></textarea>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Beschreibung</label>
                        <textarea class="form-textarea" name="description" id="descField" rows="6"
                            placeholder="Ausführliche Beschreibung..."></textarea>
                    </div>
                </div>
            </div>
            <div class="card">
                ...
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <select class="form-select" name="current_status" id="statusSelect">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" class="form-input" name="sort_order" id="sortOrder" value="0" min="0"
                            style="width:120px;">
                    </div>
                    <div class="form-group">
            ...
            <div class="card-body">
                <div class="form-group">
                    ...
                    <input type="text" class="form-input" name="meta_title" id="metaTitle"
                        placeholder="Titel für Suchergebnisse" maxlength="60">
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-textarea" name="meta_description" id="metaDescription" rows="3"
                        placeholder="Beschreibung für Suchergebnisse" maxlength="160"></textarea>
        ...
        <div class="card" style="margin-top:24px;">
            ...
            <div class="card-body">
                <div class="seo-preview">
                    ...
                    <div class="seo-preview-title" id="previewTitle">Kollaborationsname</div>
...
<!-- Step Navigation Buttons (hidden until loaded) -->
<div class="step-buttons" id="stepButtons" style="display:none;">
    <button class="btn" id="prevBtn" onclick="CollabEdit.prevStep()" style="display:none;">
        <span class="material-symbols-rounded">arrow_back</span> Zurück
    </button>
    <button class="btn btn-primary" id="nextBtn" onclick="CollabEdit.nextStep()">
...
<script>
    const CollabEdit = {
        ...
        async loadData() {
            try {
                const res = await fetch(`${this.apiBase}?action=get_collaboration&shop_id=${this.shopId}&id=${this.collabId}`);
                const data = await res.json();
                if (!data.success) {
                    document.getElementById('loadingState').innerHTML = `<p style="color:var(--error);">Fehler: ${data.error}</p>`;
                    return;
                }
                this.populateForm(data.collaboration);
            } catch (e) {
                document.getElementById('loadingState').innerHTML = `<p style="color:var(--error);">Fehler: ${e.message}</p>`;
            }
        },
        ...
        populateForm(c) {
            ...
            document.getElementById('collabSlug').value = c.slug || '';
            document.getElementById('shortDesc').value = c.short_description || '';
            document.getElementById('descField').value = c.description || '';
            document.getElementById('videoUrl').value = c.video_url || '';
            document.getElementById('sortOrder').value = c.sort_order || 0;
            document.getElementById('isFeatured').checked = c.is_featured == 1;
            document.getElementById('statusSelect').value = c.status || 'draft';
            ...
            // SEO preview
            document.getElementById('previewSlug').textContent = c.slug || 'name';
            document.getElementById('previewTitle').textContent = c.meta_title || c.name || '';
            document.getElementById('previewDesc').textContent = c.meta_description || c.short_description || '';
        ...
        renderGallery() {
            ...
            if (this.uploadedFiles.length === 0 && this.existingImages.length === 0) {
                gallery.style.display = 'none'; return;
            }
            ...
            this.existingImages.forEach((img, i) => {
                ...
                <button type="button" class="remove-btn" onclick="CollabEdit.removeExistingImage(${i})">
            ...
            this.uploadedFiles.forEach((file, i) => {
                ...
                <button type="button" class="remove-btn" onclick="CollabEdit.removeFile(${i})">
                    <span class="material-symbols-rounded" style="font-size:16px;">close</span>
                </button>`;
                gallery.appendChild(div);
            });
        ...
        removeFile(i) { this.uploadedFiles.splice(i, 1); this.renderGallery(); },
        removeExistingImage(i) {
            const img = this.existingImages.splice(i, 1)[0];
        ...
        updateVideoPreview() {
            ...
            if (!preview) return;
            if (!url) {
                preview.innerHTML = '<span class="material-symbols-rounded" style="font-size:48px;">videocam_off</span><p>Kein Video hinterlegt</p>';
                return;
            }
            let embedUrl = '';
        ...
        setupSeoListeners() {
            ...
            if (name) name.addEventListener('input', () => {
                document.getElementById('previewTitle').textContent = name.value || 'Kollaborationsname';
        ...
        // ========== SAVE ==========
        async save(statusOverride = null) {
            const form = document.getElementById('collabForm');
            const name = form.querySelector('[name="name"]').value.trim();
            if (!name) {
                const err = document.getElementById('errorName');
                if (err) { err.textContent = 'Name ist erforderlich'; err.classList.add('show'); }
                this.goToStep(1);
                return;
            }
            ...
            fd.set('status', statusOverride || document.getElementById('statusSelect').value);

            this.uploadedFiles.forEach(f => fd.append('images[]', f));
            if (this.deleteImageIds.length > 0) fd.set('delete_image_ids', JSON.stringify(this.deleteImageIds));
            const order = this.existingImages.map(img => img.id);
            if (order.length > 0) fd.set('image_order', JSON.stringify(order));
    ...
    document.addEventListener('DOMContentLoaded', () => CollabEdit.init());
</script>